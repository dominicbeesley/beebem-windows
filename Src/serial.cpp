...
void Write_ACIA_Control(unsigned char CReg) {
	...
	if ((CReg & 3)==3) {
		...
		setIRQ(SysIrq_Serial,false); // Master reset clears IRQ
		if (FirstReset) {
...
void HandleData(unsigned char AData) {
	...
	if (RIE) { setIRQ(SysIrq_Serial,true); SetACIAStatus(7); } // interrupt on receive/overun
	if (RxD<2) RxD++; 	
}
...
//	}
	...
	if ((RxD>0) && (RIE)) { setIRQ(SysIrq_Serial,true); SetACIAStatus(7); }
