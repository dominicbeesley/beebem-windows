...
void m65x_device::tick() {
	(*(void (*)(m65x_device &cpu))NextFn)(*this);
}


std::ostream& operator<<(std::ostream& o, m65x_device &dev)
{
...
bool m65x_device::execute_input_edge_triggered(int inputnum) const noexcept
{
	return inputnum == NMI_LINE;
}

void m65x_device::do_adc_d(uint8_t val)
{
	uint8_t c = P & F_C ? 1 : 0;
	P &= ~(F_N | F_V | F_Z | F_C);
...
void m65x_device::do_adc_nd(uint8_t val)
{
	uint16_t sum;
	sum = A + val + (P & F_C ? 1 : 0);
	P &= ~(F_N | F_V | F_Z | F_C);
...
void m65x_device::do_adc(uint8_t val)
{
	if (P & F_D)
		do_adc_d(val);
	else
...
void m65x_device::do_cmp(uint8_t val1, uint8_t val2)
{
	P &= ~(F_N | F_Z | F_C);
...
void m65x_device::do_sbc_d(uint8_t val)
{
	uint8_t c = P & F_C ? 0 : 1;
	P &= ~(F_N | F_V | F_Z | F_C);
...
void m65x_device::do_sbc_nd(uint8_t val)
{
...
void m65x_device::do_sbc(uint8_t val)
{
	if (P & F_D)
		do_sbc_d(val);
	else
...
void m65x_device::do_bit(uint8_t val)
{
	P &= ~(F_N | F_Z | F_V);
	uint8_t r = A & val;
	if (!r)
...
uint8_t m65x_device::do_asl(uint8_t v)
{
	P &= ~(F_N | F_Z | F_C);
	uint8_t r = v << 1;
	if (!r)
...
uint8_t m65x_device::do_lsr(uint8_t v)
{
	P &= ~(F_N | F_Z | F_C);
	if (v & 1)
		P |= F_C;
...
uint8_t m65x_device::do_ror(uint8_t v)
{
	bool c = P & F_C;
	P &= ~(F_N | F_Z | F_C);
	if (v & 1)
...
uint8_t m65x_device::do_rol(uint8_t v)
{
	bool c = P & F_C;
	P &= ~(F_N | F_Z | F_C);
	if (v & 0x80)
...
uint8_t m65x_device::do_asr(uint8_t v)
{
	P &= ~(F_N | F_Z | F_C);
	if (v & 1)
		P |= F_C;
...
void m65x_device::execute_set_input(int inputnum, int state)
{
	switch (inputnum) {
...
void m65x_device::set_nz(uint8_t v)
{
	P &= ~(F_Z | F_N);
	if (v & 0x80)
		P |= F_N;
...
void m65x_device_fetch(m65x_device &cpu) {
	cpu.PrefetchNextFn = &m65x_device_postfetch;
	m65x_device_prefetch(cpu);
}
